"""tool_call_loop：flow_agent 的 LLM 循环 —— 发送消息、解析工具调用、执行工具、回填结果。
...
def run_loop(
    ...
    返回 target_tool 执行后 result_holder 中收集的内容；
    若在 max_turns 内未调用目标工具，返回 None。

    result_holder 用于从工具副作用中提取结果（submit_tool 模式）。
    """
    messages: list[dict] = [
    ...
    for _ in range(max_turns):
        ...
        for tc in tool_calls:
            ...
            if name == target_tool and result_holder is not None and "content" in result_holder:
