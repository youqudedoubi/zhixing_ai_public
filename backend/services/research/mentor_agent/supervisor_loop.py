"""supervisor_loop：调度下级 agent 的同步 tool_call_loop。

标准流式循环 + send_message 检测：发消息给下级 → 运行下级完整循环 → 结果塞回上下文 → 继续。
"""
...
class SupervisorLoop:
    """mentor_agent 的循环：调度下级 → 拿到回复 → 继续。

    不和具体工具耦合。send_message 只是个普通工具，loop 在检测到它的结果后做分派。
    """
    ...
    def run(self, max_turns: int = 50) -> str:
        ...
        for _ in range(max_turns):
            ...
            for tr in tool_results:
                ...
                # ── 分派：send_message 发给下级 → 运行下级完整循环 ──
                if tr["name"] == "send_message":
                    args = tr.get("args", {})
                    if args.get("to") == self.subordinate_name and self.subordinate is not None:
                        reply = self.subordinate.run(args.get("message", ""))
                        self.context.append({
                            "role": "user",
                            "content": f"[来自 {self.subordinate_name}]\n{reply}",
                        })
